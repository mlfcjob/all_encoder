...
bool enable_log(m_encoder *encoder)
{
     return encoder->enable_log == 1 && encoder->fp_log != NULL;
}
...
void *x264_param_init(m_encoder *encoder)
{
	...
	if (enable_log(encoder)) {
			fprintf(encoder->fp_log, "\n***************************cpu info***********************.\n");
			fprintf(encoder->fp_log, "cpu: %08x, i_threads:%d, i_lookhead_threads:%d,"
							"b_sliced_threads:%d,"
							"b_deterministic:%d,"
							"b_cpu_independent: %d,"
							"i_sync_lookhead:%d,"
			...
			fprintf(encoder->fp_log, "\n***************************video properties************************.\n");
			fprintf(encoder->fp_log, "video properites: i_level_idc: %d.\n", param->i_level_idc);

			fprintf(encoder->fp_log, "\n***************************nal hrd************************.\n");
			fprintf(encoder->fp_log, "i_nal_hrd: %d.\n", param->i_nal_hrd);

			fprintf(encoder->fp_log, "\n****************************VUI***************************.\n");
			fprintf(encoder->fp_log, "i_sar_height:%d, i_sar_width:%d,"
			...
			fprintf(encoder->fp_log, "i_frame_reference:%d, i_dpb_size:%d, "
			...
			fprintf(encoder->fp_log, "\n****************************CQM Quant***************************.\n");
			fprintf(encoder->fp_log, "i_cqm_preset:%d, psz_cqm_file:%s.\n", param->i_cqm_preset, param->psz_cqm_file);
			fprintf(encoder->fp_log, "cqm_4iy:\n");
			int i, j;
			for(i = 0; i < 4; i++) {
					for(j = 0; j < 4; j++) {
							fprintf(encoder->fp_log, "%d, ", param->cqm_4iy[ i * 4 + j ]);
					}
					fprintf(encoder->fp_log, "\n");
			}


			fprintf(encoder->fp_log, "cqm_4py:\n");
			for(i = 0; i < 4; i++) {
					for(j = 0; j < 4; j++) {
							fprintf(encoder->fp_log, "%d, ", param->cqm_4py[ i * 4 + j ]);
					}
					fprintf(encoder->fp_log, "\n");
			}

			fprintf(encoder->fp_log, "cqm_4ic:\n");
			for(i = 0; i < 4; i++) {
					for(j = 0; j < 4; j++) {
							fprintf(encoder->fp_log, "%d, ", param->cqm_4ic[ i * 4 + j ]);
					}
					fprintf(encoder->fp_log, "\n");
			}

			fprintf(encoder->fp_log, "cqm_4pc:\n");
			for(i = 0; i < 4; i++) {
					for(j = 0; j < 4; j++) {
							fprintf(encoder->fp_log, "%d, ", param->cqm_4pc[ i * 4 + j]);
					}
					fprintf(encoder->fp_log, "\n");
			}


			fprintf(encoder->fp_log, "cqm_8iy:\n");
			for(i = 0; i < 8; i++) {
					for(j = 0; j < 8; j++) {
							fprintf(encoder->fp_log, "%d, ", param->cqm_8iy[ i * 8 + j ]);
					}
					fprintf(encoder->fp_log, "\n");
			}

			fprintf(encoder->fp_log, "cqm_8py:\n");
			for(i = 0; i < 8; i++) {
					for(j = 0; j < 8; j++) {
							fprintf(encoder->fp_log, "%d, ", param->cqm_8py[ i * 8 + j]);
					}
					fprintf(encoder->fp_log, "\n");
			}

			fprintf(encoder->fp_log, "cqm_8ic:\n");
			for(i = 0; i < 8; i++) {
					for(j = 0; j < 8; j++) {
							fprintf(encoder->fp_log, "%d, ", param->cqm_8ic[ i * 8 + j]);
					}
					fprintf(encoder->fp_log, "\n");
			}

			fprintf(encoder->fp_log, "cqm_8pc:\n");
			for(i = 0; i < 8; i++) {
					for(j = 0; j < 8; j++) {
							fprintf(encoder->fp_log, "%d, ", param->cqm_8pc[ i * 8 + j]);
					}
					fprintf(encoder->fp_log, "\n");
			}

			fprintf(encoder->fp_log, "\n****************************LOG***************************.\n");
			fprintf(encoder->fp_log, "pf_log: %p,  p_log_private: %s, i_log_level: %d, b_full_recon: %d, psz_dump_yuv: %s.\n",
							param->pf_log, param->p_log_private, param->i_log_level, param->b_full_recon, param->psz_dump_yuv);

			fprintf(encoder->fp_log, "\n****************************Encoder analasyse***************************.\n");
			fprintf(encoder->fp_log, "intra: %d, inter: %d, b_transform_8x8: %d.\n", param->analyse.intra, param->analyse.inter, param->analyse.b_transform_8x8);
			fprintf(encoder->fp_log, "i_weighted_pred: %d, b_weiget_bipred: %d.\n", param->analyse.i_weighted_pred, param->analyse.b_weighted_bipred);
			fprintf(encoder->fp_log, "i_direct_mv_pred: %d, i_chroma_qp_offset: %d.\n", param->analyse.i_direct_mv_pred, param->analyse.i_chroma_qp_offset);
			fprintf(encoder->fp_log, "i_me_method: %d, i_me_range: %d, i_mv_range: %d, i_mv_range_thread:%d.\n", param->analyse.i_me_method,
							param->analyse.i_me_range,
							param->analyse.i_mv_range,
							param->analyse.i_mv_range_thread);
			fprintf(encoder->fp_log, "i_subpel_refine: %d, b_chroma_me: %d.\n", param->analyse.i_subpel_refine, param->analyse.b_chroma_me);
			fprintf(encoder->fp_log, "b_mixed_references: %d.\n", param->analyse.b_mixed_references);
			fprintf(encoder->fp_log, "i_trellis: %d, b_fast_skip: %d.\n", param->analyse.i_trellis, param->analyse.b_fast_pskip);
			fprintf(encoder->fp_log, "b_dct_decimate: %d, i_noise_reduction: %d .\n", param->analyse.b_dct_decimate, param->analyse.i_noise_reduction);
			fprintf(encoder->fp_log, "f_psy_rd: %d, f_psy_trellis: %d.\n", param->analyse.f_psy_rd, param->analyse.f_psy_trellis);
			fprintf(encoder->fp_log, "b_psy: %d, b_mb_info: %d, b_mb_info_update: %d.\n", param->analyse.b_psy, param->analyse.b_mb_info, param->analyse.b_mb_info_update);
			fprintf(encoder->fp_log, "i_luma_deadzone[0]: %d, i_luma_deadzone[1]: %d.\n", param->analyse.i_luma_deadzone[0], param->analyse.i_luma_deadzone[1]);
			fprintf(encoder->fp_log, "b_psnr: %d, b_ssim: %d. \n", param->analyse.b_psnr, param->analyse.b_ssim);

			fprintf(encoder->fp_log, "\n****************************Rate control parameters ***************************.\n");
			fprintf(encoder->fp_log, "i_rc_method: %d, q_qp_constant: %d, i_qp_min: %d, i_qp_max: %d, i_qp_step: %d.\n", param->rc.i_rc_method, param->rc.i_qp_constant,
							param->rc.i_qp_min, param->rc.i_qp_max, param->rc.i_qp_step);
			fprintf(encoder->fp_log, "i_bitrate: %lf, f_rf_constant:%lf, f_rf_constant_max: %lf, f_rate_tolerance: %lf.\n", param->rc.i_bitrate, param->rc.f_rf_constant,
							param->rc.f_rf_constant_max, param->rc.f_rate_tolerance);
			fprintf(encoder->fp_log, "i_vbv_max_bitrate: %d, i_vbv_buffer_size: %d.\n", param->rc.i_vbv_max_bitrate, param->rc.i_vbv_buffer_size);
			fprintf(encoder->fp_log, "f_vbv_buffer_init: %lf, f_ip_factor: %lf, f_pb_factor: %lf.\n", param->rc.f_vbv_buffer_init, param->rc.f_ip_factor, param->rc.f_pb_factor);
			fprintf(encoder->fp_log, "b_filler: %d.\n", param->rc.b_filler);
			fprintf(encoder->fp_log, "i_aq_mode: %d, f_aq_strength: %lf, b_mb_tree: %d, i_lookahead: %d.\n", param->rc.i_aq_mode, param->rc.f_aq_strength, param->rc.b_mb_tree, param->rc.i_lookahead);
			fprintf(encoder->fp_log, "two pass.\n");
			fprintf(encoder->fp_log, "b_stat_write: %d, psz_stat_out: %s, b_state_wirte: %d, psz_stat_in: %s.\n", param->rc.b_stat_write, param->rc.psz_stat_out, param->rc.b_stat_read, param->rc.psz_stat_in);
			fprintf(encoder->fp_log, "two pass parameters .\n");
			fprintf(encoder->fp_log, "f_qcompress: %lf, f_qblur: %lf, f_complexity_blur: %lf, i_zones: %d, psz_ones: %s.\n", param->rc.f_qcompress, param->rc.f_qblur,
							param->rc.f_complexity_blur, param->rc.i_zones, param->rc.psz_zones);
			for(i = 0; i < param->rc.i_zones; i++) {
					fprintf(encoder->fp_log, "i_start: %d, i_end: %d, b_force_qp: %d, i_qp:%d, f_bitrate_factor: %lf. \n", param->rc.zones[i].i_start,
									param->rc.zones[i].i_end, param->rc.zones[i].b_force_qp,
									param->rc.zones[i].i_qp, param->rc.zones[i].f_bitrate_factor);
					fprintf(encoder->fp_log, "param: %p.\n\n", param->rc.zones[i].param);
			}

			fprintf(encoder->fp_log, "\n****************************Crop rect ***************************.\n");
			fprintf(encoder->fp_log, "left: %d, top: %d, right: %d, bottome: %d.\n", param->crop_rect.i_left, param->crop_rect.i_top, param->crop_rect.i_right, param->crop_rect.i_bottom);

			fprintf(encoder->fp_log, "\n****************************I frame packing ***************************.\n");
			fprintf(encoder->fp_log, "i_frame_packing: %d .\n", param->i_frame_packing);

			fprintf(encoder->fp_log, "\n****************************Muxing paramters ***************************.\n");
			fprintf(encoder->fp_log, "b_aud: %d, b_repeat_headers: %d, b_annexb %d .\n", param->b_aud, param->b_repeat_headers, param->b_annexb);
			fprintf(encoder->fp_log, "i_sps_id: %d, b_vfr_input: %d, b_pullown: %d, i_fps_num: %d, i_fps_den: %d.\n", param->i_sps_id, param->b_vfr_input,
							param->b_pulldown, param->i_fps_num, param->i_fps_den);
			fprintf(encoder->fp_log, "i_timebase_num: %d, i_timebase_den: %d.\n", param->i_timebase_num, param->i_timebase_den);

			fprintf(encoder->fp_log, "\n****************************b pic struct ***************************.\n");
			fprintf(encoder->fp_log, "b_pic_struct: %d. \n", param->b_pic_struct);

			fprintf(encoder->fp_log, "\n****************************b fake interlaced  ***************************.\n");
			fprintf(encoder->fp_log, "b_fake_interlaced: %d. \n", param->b_fake_interlaced);


			fprintf(encoder->fp_log, "\n****************************b switchable  ***************************.\n");
			fprintf(encoder->fp_log, "b_stichable: %d. \n", param->b_stitchable);

			fprintf(encoder->fp_log, "\n**************************** opencl  ***************************.\n");
			fprintf(encoder->fp_log, "b_opencl: %d, i_opencl_device: %d, opencl_device_id: %s, psz_clbin_file: %s. \n", param->b_opencl, param->i_opencl_device, param->opencl_device_id, param->psz_clbin_file );


			fprintf(encoder->fp_log, "\n**************************** slicing parameters  ***************************.\n");
			fprintf(encoder->fp_log, "i_slice_max_size: %d, i_slice_max_mbs: %d, i_slice_min_mbs: %d, i_slice_count: %d, i_slice_count_max: %d.\n",
							param->i_slice_max_size, param->i_slice_max_mbs,
							param->i_slice_min_mbs, param->i_slice_count, param->i_slice_count_max);


			fprintf(encoder->fp_log, "\n**************************** param free callback  ***************************.\n");
			fprintf(encoder->fp_log, "%p.\n", param->param_free);
    ...
    switch(encoder->format) {
        case X264_CSP_I420: {
            if (encoder->y_size *3 / 2 != input_size) {
                fprintf(encoder->fp_log, " X264_CSP_I420: invalid size.\n");
                return;
            }
        ...
        case X264_CSP_I444:{
            if (encoder->y_size != input_size) {
                fprintf(encoder->fp_log, " X264_CSP_I444: invalid size.\n");
                return;
            }
            memcpy(encoder->m_x264_pic_in->img.plane[0], input_buf, encoder->y_size);
...
static void all_fill_x265_input_buffer(m_encoder *encoder, uint8_t *input_buf, uint32_t input_size)
{
    ...
    switch(encoder->format) {
        case X265_CSP_I420:{
            if (encoder->y_size * 3 / 2 != input_size) {
                fprintf(encoder->fp_log, "X265_CSP_I420, invlid size.\n");
                return;
            }
        ...
        case X265_CSP_I444:{
            if ( encoder->y_size * 3 != input_size) {
                fprintf(encoder->fp_log, "X265_CSP_I444, invalid size.\n");
                return;
            }
    ...
    switch (codec_type) {
       case ENCODE_X264: {
           if ( NULL == (g_encoder->param = x264_param_init(g_encoder))) {
               fprintf(g_encoder->fp_log, "invalide init x264 encoder.\n");
               return NULL;
           }
           break;
       } 
       case ENCODE_X265: {
           if( NULL == (g_encoder->param = x265_param_init(g_encoder))) {
               fprintf(g_encoder->fp_log, "invlid init x265 encoder.\n");
               return NULL;
           }
           break;
       } 
    ...
    switch (encoder->codec_type) {
        ...
        case ENCODE_X265: {
            ...
            if ( NULL == encoder->codec) {
                fprintf(encoder->fp_log, "err open x265 encoder.\n");
                return;
            }
            break;
        }
...
void encoder_encode(m_encoder* encoder, uint8_t **output_buf, uint32_t *output_size)
{
    ...
    switch(encoder->codec_type) {
        case ENCODE_X264: {
            ...
            if (encode_size < 0) {
                fprintf(encoder->fp_log, "x264 encode error.\n");
                return;
            }
            ...
            *output_buf = (uint8_t*)malloc(encode_size * sizeof(uint8_t));
            ...
            for(i = 0; i < i_nal; i++) {
				...
				if(enable_log(encoder)) {
						...
						fprintf(encoder->fp_log, "size: %d.\n", p_nals[i].i_payload);
						fprintf(encoder->fp_log, "%02x, %02x, %02x, %02x. \n",  p[0], p[1], p[2], p[3]);
						fprintf(encoder->fp_log, "%lu. \n", (p[0] << 24) | (p[1] << 16) | (p[2] << 8)  | (p[3]) );
						fprintf(encoder->fp_log, "[%s]:  p_nals[%d].payload: %p, p_nals[%d].i_payload:%d.\n", __FUNCTION__, i, p_nals[i].p_payload, i, p_nals[i].i_payload);
						fprintf(encoder->fp_log, "i_type: %s.\n", nal_type[p_nals[i].i_type]);
				}
			}
            break;
        }
        case ENCODE_X265: {
            ...
            if (encode_size < 0) {
                fprintf(encoder->fp_log, "x265 encode error.\n");
                return;
            }
            ...
            *output_buf = (uint8_t*)malloc(4096 * sizeof(uint8_t));
            ...
            for(i = 0; i < i_nal; i++) {
                ...
                if (enable_log(encoder))
                    fprintf(encoder->fp_log, "[%s]:  p_nals[%d].payload: %p, p_nals[%d].sizeBytes:%d.\n", __FUNCTION__, i, p_nals[i].payload, i, p_nals[i].sizeBytes);
...
void encoder_close(m_encoder*  encoder)
{
    ...
    if (NULL != encoder->fp_log) {
        printf("close log file.\n");
        fclose(encoder->fp_log);
    }
}
